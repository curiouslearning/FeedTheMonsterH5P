...
const PuzzelBar = (props: any) => {
  ...
  return <div className='puzzel-indicator-container' style={{backgroundImage: `url(${indicatorBg})`,backgroundRepeat: "no-repeat", backgroundSize: 160}}>
    {
...
export const PuzzelBarHook = (puzzelCount: any, activeIndicators: any) => {
  ...
  const puzzelsIndicators = () => {
    ...
    for(let i = 0; i < puzzelCount; i++) {
      puzzels.push(<div className={classNames('completed-puzzel', {'active-indicator' : i < activeIndicators})} key={i} style={{backgroundImage: (i < activeIndicators) ? `url(${fullBarBg})`: `url(${emptyBarBg})`,backgroundRepeat: "no-repeat", backgroundSize: "contain"}}></div>)
    }
